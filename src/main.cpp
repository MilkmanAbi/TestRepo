...
int main(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--upgrade") == 0) {
            // OIU hook: checked in order (system install → user install → beside binary)
            const char* candidates[] = {
                "/usr/local/bin/.ytcui-oiu-upgrade",
                nullptr
            };
            char hook_beside[4096] = {0};
            char self_dir[4096]    = {0};
            if (readlink("/proc/self/exe", self_dir, sizeof(self_dir)-1) > 0) {
                // dirname in-place
                char* sl = strrchr(self_dir, '/');
                if (sl) { *sl = '\0'; }
                snprintf(hook_beside, sizeof(hook_beside),
                         "%s/.ytcui-oiu-upgrade", self_dir);
            }

            const char* found = nullptr;
            for (int c = 0; candidates[c]; c++) {
                if (access(candidates[c], X_OK) == 0) { found = candidates[c]; break; }
            }
            if (!found && hook_beside[0] && access(hook_beside, X_OK) == 0)
                found = hook_beside;

            if (found) {
                printf("Launching OIU updater...\n\n");
                execl("/bin/bash", "bash", found, nullptr);
                perror("Failed to launch updater");
                return 1;
            } else {
                fprintf(stderr, "OIU upgrade hook not found.\n");
                fprintf(stderr, "Reinstall: git clone <repo> && ./oiu/oiu install\n");
                return 1;
            }
        }
    }
