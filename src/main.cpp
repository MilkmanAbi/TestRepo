    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (strcmp(argv[i], "--ois")          == 0 ||
            ...
            if (n > 0) {
                ...
                if (sl) *sl = '\0';
                snprintf(hook, sizeof(hook), "%s/.ytcui-ois", self_dir);
            }
            if (!hook[0] || access(hook, X_OK) != 0)
                snprintf(hook, sizeof(hook), "/usr/local/bin/.ytcui-ois");
            ...
            fprintf(stderr,
                "\033[33mOIS not found. Reinstall:\033[0m\n"
                "  git clone https://github.com/MilkmanAbi/TestRepo\n"
                "  cd TestRepo && sh install.sh\n\n");
            return 1;
        }
    }
